    ...
    <div className="bg-gray-100 dark:bg-gray-900 min-h-screen flex items-center justify-center px-4 py-10">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg w-full max-w-md p-6 md:p-10">
        <div className="text-center mb-6 md:mb-8">
          <h1 className="text-2xl md:text-3xl font-bold text-gray-800 dark:text-white">{t('auth.welcome_back')}</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-2 text-sm md:text-base">{t('auth.login_subtitle')}</p>
        </div>
        ...
        <p className="text-center text-gray-500 dark:text-gray-400 mt-6 text-sm">
          {t('auth.no_account')}{' '}
          <Link to="/register" className="text-yellow-500 hover:underline font-medium">{t('auth.register_link')}</Link>
        </p>
      </div>
    </div>
  );
